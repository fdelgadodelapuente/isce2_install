...
*Caption:* Sampling frequencies of different SAR satellites. Here B$_{p
...
[H]
\includegraphics{range_res_Lband.pdf}
...
*Caption:* SLC images resolution by platform compared with typical available DEMs. $\Delta R_{r
...
The stripmapApp steps are
    ...
    \item \texttt{startup}:
    \item \texttt{preprocess}: extract the SLCs.
    \item \texttt{cropraw}: crop the raw images.
    \item \texttt{formslc}: focus SLC with range-doppler algorithm, skipped for SLCS zero-doppler data.
    \item \texttt{cropslc}: crop the SLCs.
    \item \texttt{verifyDEM}: check if external DEM is provided, otherwise it downloads SRTM and convert the elevation from the EGM96 geoid to the WGS84 ellipsoid. 
    \item \texttt{topo}: project DEM to range-azimuth coordinates using the range Doppler equation and some Newton-Raphson solvers.
    \item \texttt{geo2rdr}: calculate range and azimuth offsets with geometric coregistration.
    \item \texttt{coarse\_resample}: coregister secondary SLC with geometric coregistrationonly.
    \item \texttt{misregistration}: run ampcor on coarsely registered secondary SLC and estimate the average  range and azimuth offset to improve the coregistration.
    \item \texttt{refined\_resample}: coregister again the coarse registered secondary SLC with the average azimuth and range offsets from ampcor.
    \item \texttt{dense\_offsets*}: optional step. Run ampcor on coregistered SLCs to extract range and azimuth offsets. These dense offsets can be used for two things. First, if the ionosphere is strong enough to introduce artificial azimuth offsets due to an azimuthal variation in the ionospheric TEC, then the geometric registration is not enough to align the SLCs. In that case the images need to be aligned using variable range and azimuth offsets like in ROI\_PAC. Second, they can be used to calculate horizontal displacements for pixel tracking.
    \item \texttt{rubber\_sheet*}: the dense azimuth offset is addedd to the geometric azimuth offset to improve the SLC coregistration.
    \item \texttt{fine\_resample}: coregister the secondary SLC to the reference using the average offsets. If rubbe\_sheet was carried out, include the fine azimuth offset field ampcor to correct for potential azimuth misalignments.
    \item \texttt{split\_range\_spectrum*}: calculate upper and lower band interferograms by range filtering.
    \item \texttt{sub\_band\_resample*}: interpolate the secondary sub-band SLCs with the geometric coregistration (from \texttt{topo}) and constant range and azimuth offset (from \texttt{misregistration}).
    \item \texttt{interferogram}: cross multiply reference with finely registered references SLC. Also remove the flat earth and topographic phase with the range offsets file, and apply looks. The interferogram generation step is a simple cross multiplication of reference and coregistered SLCs. The range offsets also represent the phase that is used for flattening. So all 3 pieces are combined together during cross multiplication.
    \item \texttt{sub\_band\_interferogram*}: calculate low and high band interferograms.
    \item \texttt{filter}: filter the interferogram with the Goldstein power spectrum filter.
    \item \texttt{filter\_low\_band*}: filter the low band interferogram.
    \item \texttt{filter\_high\_band*}: filter the high band interferogram.
    \item \texttt{unwrap}: unwrap the phase with icu, grass, snaphu, snaphu\_mcf.
    \item \texttt{unwrap\_low\_band*}: unwrap the filtered low band interferogram.
    \item \texttt{unwrap\_high\_band*}: unwrap the filtered high band interferogram.
    \item \texttt{ionosphere*}: calculate the ionospheric correction with formula 7 of \citep{Fattahi2017a}.
    \item \texttt{geocode}: geoode all prodcuts.
    \item \texttt{geocodeoffsets}: optional step for pixel tracking only, geocode range and azimuth offsets.
    \item \texttt{endup}
...
Here I only describe the steps that are different than those in stripmapApp.py
    ...
    \item \texttt{computeBaselines }
    \item \texttt{verifyDEM }
    \item \texttt{topo}: same as stripmapApp but for each individual burst
    \item \texttt{subsetoverlaps}: extract the burst overlap regions 
    \item \texttt{coarseoffsets}: calculate coarse offsets with ampcor in the burst overlap regions for the upper and lower bursts 
    \item \texttt{coarseresamp}: interpolate the secondary burst SLC to the reference one with the coaarse offset field
    \item \texttt{overlapifg}: calculate differential and upper and lower interferograms
    \item \texttt{prepesd}: calculate frequency difference for ESD
    \item \texttt{esd}: enhanced spectral diversity correction for the accurate azimuth misregistration in the burst overlap regions
    \item \texttt{rangecoreg}: calculate range offsets only with ampcor for fine coregistration
    \item \texttt{fineoffsets}: calculate geometric offsets for every pair of bursts
    \item \texttt{fineresamp}: interpolate the secondary burst SLC to the reference one with the geometric offset, fine range offsets and ESD
    \item \texttt{ion}: calculate ionospheric correction for burst overlap regions. This might be necessary if you work in a low geomagnetic latitude and if you observe phase discontinuities in the burst overlap regions after accounting for ESD.
    \item \texttt{burstifg}: calculate interferograms for every bursts
    \item \texttt{mergebursts}: merge the burst interferograms
    \item \texttt{filter }
    \item \texttt{unwrap }
    \item \texttt{unwrap2stage}: use a linear programming solver to solve for the constants between the connected componets file of the phase unwrappoint output
    \item \texttt{geocode}
    \item \texttt{denseoffsets }
    \item \texttt{filteroffsets }
    \item \texttt{geocodeoffsets}
...
*Caption:* Output files of \texttt{topsApp.py
...
The interferogram is calculated in three steps. First, subtract the phase. Second remove the flat earth. Third, remove the simulation. This specific part of the workflow is more akin to process\_2pass.pl in ROI\_PAC than to  stripmapApp.py
    ...
    \item \texttt{preprocess}
    \item \texttt{baseline}
    \item \texttt{download\_dem}
    \item \texttt{prep\_slc}
    \item \texttt{slc\_offset}: coregister SLCs with ampcor. Unlike \texttt{stripmapApp.py}, \texttt{alos2App.py} doesn't use geometry to align the SLC's.
    \item \texttt{form\_int}: calculate interferogram
    \item \texttt{swath\_offset}: for ScanSAR only
    \item \texttt{swath\_mosaic}: for ScanSAR only
    \item \texttt{frame\_offset}: for ScanSAR only
    \item \texttt{frame\_mosaic}: for ScanSAR only
    \item \texttt{rdr2geo}: calculate geometry and simulation in radar coordinates for multilooked interferogram. This is similar to the topo step in \texttt{stripmapApp.py} and \texttt{topsApp.py}
    \item \texttt{geo2rdr}: calculate synthetic phase from geometry and simulation 
    \item \texttt{rdrdem\_offset}: cross correlate DEM with flattened interferogram
    \item \texttt{rect\_rgoffset}: rectify range offset file to reference SLC.
    \item \texttt{diff\_int}: calculate differential interferogram
    \item \texttt{look}
    \item \texttt{coherence}
    \item \texttt{ion\_subband}: split the range spectrum in two bands from the carrier frequency. This result in a low band and an high band interferogram 
    \item \texttt{ion\_unwrap}: unwrap sub-band interferograms
    \item \texttt{ion\_filt}: filter unwrapped sub-band interferograms
    \item \texttt{ion\_correct}: decompose the phase into dispersive and non-dispersive components from the unwrapped sub-band interferograms and remove dispersive component from the differential interferogram
    \item \texttt{filt}
    \item \texttt{unwrap}
    \item \texttt{geocode}
    \item \texttt{slc\_mosaic}
    \item \texttt{slc\_match}
    \item \texttt{dense\_offset}
    \item \texttt{filt\_offset}
    \item \texttt{geocode\_offset}


### SM3 and SM1 processing in \texttt{alos2App.py compared with \texttt{stripmapApp.py}}
...
[H]
\includegraphics{pairs_alos_p114.pdf}
...
*Caption:*  [a] ALOS interferogram of the Mw 7.0 Pichilemu earthquake with ionospheric streaks \texttt{(Igrams/20100309\_20100424/filt\_20100309\_20100424\_snaphu.unw)
...
[H]
\includegraphics{pairs_envi_asc_320.pdf}

*Caption:* Perpendicular baseline plot for ENVISAT IM2 ascending track 320 at Yellowstone caldera (\citep{Delgado2021a
...
[H]
\includegraphics{pairs_rs2_u16w2.pdf}
...
[H]
\includegraphics{pairs_csk_villarrica.pdf}

*Caption:* Perpendicular baseline plot for COSMO-SkyMed descending data for Villarrica volcano (\citep{Delgado2017
...
[H]
\includegraphics{alos2_saocom_Bp.pdf}

*Caption:* Perpendicular baseline plot for ALOS-2 SM3 and SAOCOM-1 stripmap data (\citep{Delgado2024
...
*Caption:* Perpendicular baseline plot for Sentinel-1 descending track 83 for Cordon Caulle volcano (\citep{Delgado2021a
...
%%%%%### April 2009 M$_{W$ 6.3 L'Aquila, normal faulting}
%%%%%
...
### April 2016 M$_{W$ 7.8 Pedernales}
...
%%### May 2021 M$_{W$ 7.3 Maduo, strike-slip faulting}
%%### Jan 2022 M$_{W$ 6.6 Menyuan, strike-slip faulting}
...
% 
%     \item Radar $\,\to\,$ Coregistration $\,\to\,$ S-1 TOPS Coregistration
%     \item TOPSAR-Split $\,\to\,$  select the IW1 subswath for each of the products Select bursts 1-3 for 150414 SLC and bursts 4-6 for 150426 SLC Apply-Orbit-File $\,\to\,$  Sentinel Precise Orbit State Vectors
    
%     \item Radar $\,\to\,$ Interferometric $\,\to\,$ Products $\,\to\,$  Interferogram Formation. Use \texttt{\_orb\_stack} product
    
%     \item Radar $\,\to\,$ Sentinel-1 TOPS $\,\to\,$  S-1 TOPS Deburst. Use \texttt{\_orb\_stack\_ifg} product
    
%     \item Radar $\,\to\,$ Interferometric $\,\to\,$ Products $\,\to\,$ Topographic Phase Removal. Use \texttt{\_orb\_stack\_ifg\_deb} product
    
%     \item Radar $\,\to\,$ Multilooking Processing parameters, Number of Range Looks, 10 looks or 20. Always use GR Square Pixel Interferograms with short temporal baselines can be processed with 10 looks, and those that are noisier should have a larger number of looks. Use \texttt{\_orb\_stack\_ifg\_deb\_dinsar} product
    
%     \item  Radar $\,\to\,$ Interferometric $\,\to\,$ Filtering $\,\to\,$ Goldstein Phase Filtering (Processing Parameters $\,\to\,$ Adaptative Filter Exponent $\,\to\,$ 0.5). Use \texttt{\_orb\_stack\_ifg\_deb\_dinsar\_ML} product
    
%     \item Radar $\,\to\,$ Geometric $\,\to\,$ Terrain Correction $\,\to\,$ Range-Doppler Terrain Correction (Intensity and Phase). Use \texttt{\_orb\_stack\_ifg\_deb\_dinsar\_ML\_flt} product
% 
